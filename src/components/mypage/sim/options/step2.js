...
class Sim_Options_Step2 extends ComponentBase {
  constructor(props) {
    super(props)
    ...
    this.state = {
      ...
      gender: '', // オプション変更時はapplyInfoに入ってこない
    }
  }
  async componentDidMount() {
    ...
    const { customerInfo } = await getAgreementData(window.customerId, true)
    this.setState({ formState, gender: customerInfo.gender })
  }
  ...
  handleChangeCertificateType(e) {
    ...
    if (value === '101') {
      if (this.state.gender == 3 && verified.length < 8) {
  ...
  goNextDisplay(e, url, params) {
    ...
    if (url === '/mypage/sim/options/confirm') {
  ...
  render() {
    const { formState } = this.state
    const isCorporation = this.state.gender == 3 ? true : false
    ...
    const disabledImageListNum = this.state.gender == 3 ? 49 : 3
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        if (this.validateAllForm()) {
                          /*  this.goNextDisplay(e, '/mypage/sim/options/confirm') */
...
export default connect(mapStateToProps)(Sim_Options_Step2)
