...
class Change_Name_Step2 extends ComponentBase {
  constructor(props) {
    super(props)
    ...
    this.state = {
      ...
      defaultName:
        props.history.location.state !== undefined
          ? props.history.location.state.defaultName
          : {
              lastName: '',
              firstName: '',
              lastNameKana: '',
      ...
      isChangeUser_kana: false, //同名の場合のかな表記
      isChangeUser_iccid: '', //変更対象iccid
        ...
        if (IMAGE_TYPES.indexOf(imageType) !== -1) {
          ...
          reader.onload = function (f) {
            ...
            if (this.state.formState.imageInfo.certificateTypeCode === '101') {
              this.setState({
                selectedImageData: f.target.result,
              })
              ...
              image.onload = function () {
                ...
                this.setState({ editMaskModal: true, selectedImageSize })
              }.bind(this)
            } else {
              // API
  ...
  goNextDisplay(e, url, params) {
    ...
    if (url === '/mypage/change/name/confirm') {
      const params = {
  ...
  handleSubmit() {
    ...
    //名前に変更がない場合はreturn
    if (
      this.state.defaultName.lastName ==
        this.state.formState.customerInfo.lastName &&
      this.state.defaultName.firstName ==
        this.state.formState.customerInfo.firstName &&
      this.state.defaultName.lastNameKana ==
        this.state.formState.customerInfo.lastNameKana &&
      this.state.defaultName.firstNameKana ==
    ...
    /* const testData = moment('20221010')
    const testData2 = moment('20221011')
    console.log(testData.isSame(today, 'day'))
    console.log(testData2.isSame(today, 'day')) */
    // 変更時のみ判別処理
    let isChangeUser = false
    let isChangeUser_kana = false
    let isChangeUser_iccid = ''

    if (this.state.simInfo) {
      this.state.simInfo.forEach((item) => {
        const isRemoveData =
        ...
        const isDiffName =
          ...
          item.name.replaceAll(/\s+/g, '') !==
            this.state.formState.customerInfo.lastName +
              this.state.formState.customerInfo.firstName
        const isDiffNameKana =
          (this.state.defaultName.lastNameKana !==
            this.state.formState.customerInfo.lastNameKana ||
            this.state.defaultName.firstNameKana !==
              this.state.formState.customerInfo.firstNameKana) &&
          item.kana.length > 0 &&
          item.kana.replaceAll(/\s+/g, '') !==
            this.state.formState.customerInfo.lastNameKana +
              this.state.formState.customerInfo.firstNameKana

        if (!isRemoveData && isDiffName) {
          isChangeUser = true
          isChangeUser_iccid = item.ICCID
        } else if (!isRemoveData && isDiffNameKana) {
          isChangeUser_kana = true
          isChangeUser_iccid = item.ICCID
        }
      })
      if (isChangeUser) {
        this.setState({ changeUserModal: true })
        this.setState({ isChangeUser_iccid })
      } else if (isChangeUser_kana) {
        this.setState({ changeUserModal: true })
        this.setState({ isChangeUser_iccid })
        this.setState({ isChangeUser_kana: true })
      } else {
        this.handleSubmitSend()
      }
    }
  ...
  handleChangeUser(value) {
    if (value) {
      let formState = Object.assign({}, this.state.formState)
      const simList = [
        {
          iccid: this.state.isChangeUser_iccid,
          userInfoChangeFlag: 1,
        },
      ]
      formState.simList = simList
      this.setState({ formState })
    }
    this.handleSubmitSend()
  }
  ...
  render() {
    const { formState } = this.state
    const isCorporation = formState.customerInfo.gender == 3 ? true : false
    console.log(formState)

    const certificateOptions_list = isCorporation
      ? corporationCertificateOptions
      : certificateOptions
                  ...
                  <div className="ui_field">
                    <div className="ui_select">
                      <select onChange={this.handleChangeCertificateType}>
                        ...
                        {certificateOptions_list.map((item) => {
                          const selected =
                            item.value ==
                ...
                >
                  ...
                  <li>
                    <label className="ui_checkbox">
                      ...
                      />
                      <span className="ui_checkbox_label">
                        {formState.customerInfo.gender == 3 ? '法人名' : '氏名'}
                        は正しい
                      </span>
                    </label>
                  </li>
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        if (this.validateAllForm()) {
                          /*  this.goNextDisplay(e, '/mypage/change/name/confirm') */
                          this.handleSubmit()
                        } else {
          ...
          <div className="ulModal">
            <div className="ulModal_main">
                  ...
                  {this.state.isChangeUser_kana ? (
                    ...
                    <React.Fragment>
                      ...
                      {this.state.defaultName.lastName +
                        this.state.defaultName.firstName}
                      」さんがいらっしゃいますが、
                      <br />「{this.state.formState.customerInfo.lastName}{' '}
                      {this.state.formState.customerInfo.firstName}
                      」さんに変更しますか？
                    </React.Fragment>
                  )}
              ...
              >
                <button
                  className="formbtn return"
                  onClick={() => this.handleChangeUser(false)}
                >
                  更新しない
                </button>
                <button
                  className="formbtn next"
                  onClick={() => this.handleChangeUser(true)}
                >
                  更新する
                </button>
              </div>
...
export default connect(mapStateToProps)(Change_Name_Step2)
